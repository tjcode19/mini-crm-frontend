<template>
  <div id="login-form">
    ...
    <p class="text-center text-gray-500 text-xs">&copy;2020 Mini CRM. tjcode19</p>
  </div>
</template>
  ...
  methods: {
    handleSubmit() {
      ...
      if (this.login.username != "" && this.login.password != "") {
        this.loading = true;
        HTTP.post("auth/login", this.login).then((request) =>
    ...
    loginFailed() {
      this.loading = false;
      console.log(
        "Login Failed, Msg: " + this.errorMsg + "Error: " + this.error
      );
